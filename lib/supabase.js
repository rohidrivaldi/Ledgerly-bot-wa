...
export const getUserByPhone = async (phoneNum) => {
  try {
    ...
    // Mapping LID owner ke nomor HP asli jika dikirim via companion device (LID)
    let targetPhone = phoneNum;
    if (config.owner && config.owner.lid && phoneNum === config.owner.lid) {
      targetPhone = config.owner.number;
    }

    // Supabase menyimpan noTelp sebagai numeric/bigint (contoh: 6285750917686)
    const phoneInt = parseInt(targetPhone);
    if (isNaN(phoneInt)) return null;

    // Hanya cari user dengan role = 'pemilik' agar tidak bentrok dengan akun superadmin
